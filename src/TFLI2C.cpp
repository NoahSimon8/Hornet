...
#include "hardware/TFLI2C.h"       //  TFLI2C library header

// Constructor/Destructor
TFLI2C::TFLI2C() : _wire(&Wire) {} 
TFLI2C::~TFLI2C(){}
...
bool TFLI2C::readReg( uint8_t nmbr, uint8_t addr)
{
  ...
  _lastErr = err;
  if( err != 0)  // If write error...
  { 
    Serial.println("I2C readReg error");
  ...
  _wire->requestFrom( ( int)addr, 1, true);
    ...
    {
    Serial.println("I2C readReg peek error");
...
bool TFLI2C::writeReg( uint8_t nmbr, uint8_t addr, uint8_t data)
{
  ...
  if( err!= 0)  // If write error...
  {
    Serial.println("I2C writeReg error");
